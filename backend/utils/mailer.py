...
def send_email_verification_email(
    to_email: str, verification_link: str, first_name: Optional[str] = None
) -> bool:
    """
    Send the official Firebase email-verification link via our SMTP.
    The link is generated by Firebase Admin (oobCode); one click completes verification in the browser.
    """
    link = (verification_link or "").strip()
    if not link:
        return False
    name = (first_name or "").strip()
    greet = f"Hi {name}," if name else "Hi,"
    subject = "Verify your email — Mkulima Sokoni"
    text = "\n".join(
        [
            greet,
            "",
            "Thanks for joining Mkulima Sokoni.",
            "",
            "Please verify your email address by opening this link once:",
            "",
            link,
            "",
...
<p style="margin:0 0 14px;color:#374151;line-height:1.6;">{greet} Thanks for joining Mkulima Sokoni. Tap the button below to confirm your email address and unlock full access.</p>
{_button_html('Verify my email', link)}
{_plain_link_html('Or copy this link', link)}
<div style="margin-top:18px;color:#6b7280;font-size:13px;">If you did not sign up, you can ignore this message.<br /><strong>Mkulima Sokoni Team</strong></div>
"""
    html = _email_base_html("Verify your email", "Confirm your email for Mkulima Sokoni.", html_body)
    return send_email(to_email, subject, text, html=html)
